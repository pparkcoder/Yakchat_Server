...
public class QuestionService {
	...
	public List<QuestionWithAnswersDto> getQuestionsByUserIdByCreatedAtDesc(Long userId) {
		...
		// 내가 작성한 질문 조회
		List<QuestionWithAnswersDto> questions = questionRepository.findTop5ByUserIdOrderByCreatedAtDesc(userId)
			.stream()
		...
		// 질문 별 답변 개수 count
		Map<Long, Long> answerCount = answers.stream()
			.collect(Collectors.groupingBy(AnswerDto::getQuestionId, Collectors.counting()));
	...
	public List<QuestionWithAnswersDto> getQuestionsByUserIdByAcceptedByCreatedAtDesc(Long userId) {
		...
		// 질문 별 채택된 답변 조회
		List<AnswerDto> answers = answerRepository.findByQuestionIdAndIsAcceptedInOrderByCreatedAtDesc(questionIds,
				true).stream()
			.map(this::convertAnswerToDto)
			.collect(Collectors.toList());
